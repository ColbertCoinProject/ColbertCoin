...
uint256 hashGenesisBlock("0x465aadfdb80f1fb9e985bafa37def53c18cbc7cdde9fa294f4df6ade6a4a7aba");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // ColbertCoin: starting difficulty is 1 / 2^12
...
{
    // ColbertCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // ColbertCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // ColbertCoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // ColbertCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // ColbertCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // ColbertCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // ColbertCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x465aadfdb80f1fb9e985bafa37def53c18cbc7cdde9fa294f4df6ade6a4a7aba");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "20/Feb/2014 Haunting sketches show horror of North Korean prison camps";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1392948027;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1787254;
        ...
        assert(block.hashMerkleRoot == uint256("0xd3d4e5681ea179f65dde139dd377af33708533eca18b8d2b1435080141b2300c"));

#ifdef _MAKE_GENESIS_BLOCK
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // ColbertCoin: increase each by adding 2 to bitcoin's value.
...
void static ColbertCoinMiner(CWallet *pwallet)
{
    printf("ColbertCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ColbertCoin-miner");
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ColbertCoinMiner, pwallet));
}
